...
dataconfig = load_toml('dataconfig.smacv2-ablation.return.toml')
...
plotter = plotter.scale(
    color=so.Nominal(
        make_color_values(),
        **load_toml('plotconfig.color-order.smacv2-ablation.toml'),
    )
)

limits = load_toml('plotconfig.xlimit.smacv2.toml')
limits |= load_toml('plotconfig.ylimit.smacv2.return.toml')
plotter = plotter.limit(**limits)

plotter = plotter.label(**make_label(keys, flags['normalize']))

plotter = plotter.facet(**load_toml('plotconfig.facet.smacv2-ablation.toml'))
plotter = plotter.layout(size=(10, 3.5), engine='tight')
plotter = plotter.plot()

hack_legend(plotter._figure)

path = pathlib.Path('plots/qfix.smacv2-ablation.return-mean.pdf')
