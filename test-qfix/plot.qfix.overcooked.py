...
dataconfig = load_toml('dataconfig.overcooked.toml')
...
plotter = plotter.scale(
    color=so.Nominal(
        make_color_values(),
        **load_toml('plotconfig.color-order.overcooked.toml'),
    )
)

limits = load_toml('plotconfig.xlimit.overcooked.toml')
limits |= load_toml('plotconfig.ylimit.overcooked.normalized.toml')
plotter = plotter.limit(**limits)

plotter = plotter.label(**make_label(keys, flags['normalize']))

plotter = plotter.facet(**load_toml('plotconfig.facet.overcooked.toml'))
plotter = plotter.layout(size=(10, 2), engine='tight')
plotter = plotter.plot()

hack_legend(plotter._figure)

path = pathlib.Path('plots/qfix.overcooked.pdf')
path.parent.mkdir(parents=True, exist_ok=True)
plotter.save(path, bbox_inches='tight')

remove_legends(plotter._figure)

path = pathlib.Path('plots/qfix.overcooked.nolegend.pdf')
path.parent.mkdir(parents=True, exist_ok=True)
plotter.save(path, bbox_inches='tight')
